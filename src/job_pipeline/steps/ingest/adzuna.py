...
class AdzunaAPI:
    ...
    def search_jobs(
        ...
        page: int = 1,  # goes with single_page single_thread
        ...
        Args:
            ...
            formatted: If True, returns formatted job results; otherwise raw API results
            page_list: List of specific page numbers to fetch (only for "page_list" scope)

        Returns:
            Tuple containing:
                - List of job results (formatted or raw based on 'formatted' param)
        ...
        if mode not in ["single_thread", "multithreading"]:
            raise ValueError(
                f"Invalid mode '{mode}'. Must be 'single_thread' or 'multithreading'"
            )

        if scope not in ["single_page", "all_pages", "page_list"]:
            raise ValueError(
                f"Invalid scope '{scope}'. Must be 'single_page', 'all_pages', or 'page_list'"
            )

        if results_per_page > 50:
            raise ValueError("results_per_page cannot exceed 50")
        ...
        return (
            self._process_job_results(results) if formatted else results
        ), page_error_list
    ...
    def _fetch_single_page(
        ...
        try:
            ...
            log.error(f"page: {page}")
            return {"error": str(e), "error_page": page}
        except requests.exceptions.RequestException as e:
            log.error(f"Error making request to Adzuna API: {e}")
            return {
                "error": str(e)
